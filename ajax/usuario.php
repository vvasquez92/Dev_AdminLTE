...
switch ($_GET["op"]) {
	...
	case 'listar':
		...
		while ($reg = $rspta->fetch_object()){
			$data[] = array(
					"0"=>($reg->condicion)?
					'<button class="btn btn-warning btn-xs" onclick="mostar('.$reg->iduser.')"><i class="fa fa-edit"></i></button>'.
					' <button class="btn btn-danger btn-xs" onclick="desactivar('.$reg->iduser.')"><i class="fa fa-times"></i></button>':
					'<button class="btn btn-warning btn-xs" onclick="mostar('.$reg->iduser.')"><i class="fa fa-edit"></i></button>'.
